...
def insert_position_to_db(position):
    ...
    cursor.execute('''
        ...
        VALUES (?, ?, ?, ?, ?)
    ''', (position['latitude'], position['longitude'], position['altitude'], position['ts_utc'], position['day']))
    conn.commit()
    conn.close()
...
# Background thread to fetch every 1 second
...
def get_last_3_days():
    ...
    # Sample: 1 record per minute to reduce payload
    sampled = rows[::60]
    data = [{'latitude': r[0], 'longitude': r[1], 'altitude': r[2], 'ts_utc': r[3], 'day': r[4]} for r in sampled]
    return jsonify(data)
...
def get_stats():
    ...
    rows = cursor.fetchall()
    conn.close()
    return jsonify({'records_per_day': {r[0]: r[1] for r in rows}, 'collection_rate': f'{UPDATE_INTERVAL} sec/request', 'max_days': MAX_DAYS})
